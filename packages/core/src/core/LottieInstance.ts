...
import { WebGLRenderer, isWebGLSupported } from './WebGLRenderer'
...
export class LottieInstance implements ILottieInstance {
  ...
  // 渲染器切换
  private currentRenderer: 'svg' | 'canvas' | 'html' | 'webgl' = 'svg'
  ...
  switchRenderer(renderer: 'svg' | 'canvas' | 'html' | 'webgl'): void {
    ...
    // 检查 WebGL 支持
    if (renderer === 'webgl' && !isWebGLSupported()) {
      console.warn('[LottieInstance] WebGL not supported, falling back to canvas')
      renderer = 'canvas'
    }
    ...
    try {
      ...
      } else if (this.webglRenderer && this.currentRenderer === 'webgl') {
        // 从 WebGL 切换到其他渲染器
      ...
      // 更新配置
      this._config.renderer = renderer === 'webgl' ? 'canvas' : renderer // WebGL 使用 canvas 作为基础
  ...
  private initWebGLRenderer(): void {
    ...
    try {
      ...
      console.log('[LottieInstance] WebGL renderer initialized')
    } catch (error) {
